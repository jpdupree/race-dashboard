...
// Exposes window.Race = { fmt, slug, config, gh, course, compute, gpx, qs }.
...
(function () {
  ...
  //   proxy mode:   race-hub-session-v1 = { proxyUrl, session, username, role, expiresAt }
  ...
  const auth = {
    ...
    cfg() {
      if (auth.mode() === 'proxy') {
        const s = auth.loadSession();
        if (!s) return null;
        return { mode: 'proxy', proxyUrl: s.proxyUrl, session: s.session, username: s.username };
      }
      const c = config.load();
      if (!c) return null;
  ...
  const course = {
    ...
    legCount(cfg) {
      if (!cfg) return 0;
      if (cfg.courseType === 'loops') return cfg.course?.loopCount || 0;
      if (cfg.courseType === 'segments') return (cfg.course?.segments || []).length;
      return 0;
    },

    // Returns the leg definition for a 1-based index, normalized:
    // { name, fromAid?, toAid?, distanceMi, elevationGainFt?, elevationLossFt?,
    //   arriveCutoffHours?, cumulativeMi }
    legAt(cfg, index1) {
      const i = index1 - 1;
      if (!cfg || i < 0) return null;
      if (cfg.courseType === 'loops') {
        const c = cfg.course || {};
        if (index1 > (c.loopCount || 0)) return null;
        return {
          index: index1,
          name: `Lap ${index1}`,
          ...
          arriveCutoffHours: index1 === (c.loopCount || 0) ? (cfg.cutoffs?.totalHours || null) : null,
          cumulativeMi: index1 * (c.loopDistanceMi || 0)
        };
      }
    ...
    totalDistanceMi(cfg) {
      if (!cfg) return 0;
      if (cfg.courseType === 'loops') {
        return (cfg.course?.loopCount || 0) * (cfg.course?.loopDistanceMi || 0);
      }
      if (cfg.courseType === 'segments') {
    ...
    // For segments mode: derive [{name, mileage}] aid stations from
    // consecutive fromAid/toAid pairs. Start aid is fromAid of seg[0];
    // each subsequent aid is toAid of the previous segment.
    aidStations(cfg) {
      if (!cfg || cfg.courseType !== 'segments') return [];
      const segs = cfg.course?.segments || [];
  ...
  // ---------- expose ----------
  window.Race = { fmt, slug, qs, hub, config, auth, gh, course, compute, gpx };
})();
