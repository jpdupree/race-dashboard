...
(function () {
  ...
  const charts = {
    ...
    render(container, cfg, data) {
      ...
      container.innerHTML =
        ...
        card('Mile pace', 'Per-leg pace (min / mi)', 'chart-pace') +
        ...
        '</div>' +
        card('Cumulative progress', 'Miles covered over race time; dashed = projected finish, cutoff line shown', 'chart-cum');
      ...
      charts.lineChart(svg('chart-pace'), paceSeries, {
        ...
        yFmt: s => fmt.pace(s), xFmt: x => 'L' + Math.round(x),
        xTicks: Math.min(legCount, 10)
      });
      ...
      charts.lineChart(svg('chart-cum'), cumSeries, {
        ...
        cutoff: totalDist, yFmt: y => y.toFixed(0) + 'mi', xFmt: x => x.toFixed(0) + 'h',
        dots: false
      });
    }
  };
  ...
  // ---------- expose ----------
  window.Race = { fmt, slug, qs, hub, config, auth, share, api, gh, course, compute, gpx, charts, mountAccountWidget };
})();
